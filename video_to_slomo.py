...
parser.add_argument("--imgs", type=str,
                    help='path of imgs to be converted')
...
parser.add_argument("--output", type=str, required=True,
                    help='Specify output file name or path')
...
def extract_frames(video, outDir):
    ...
    error = ""
    print('{} -i {} -vsync 0 -qscale:v 2 {}/%06d.jpg'.format(
        os.path.join(args.ffmpeg_dir, "ffmpeg"), video, outDir))
    retn = os.system('{} -i {} -vsync 0 -qscale:v 2 {}/%06d.jpg'.format(
        os.path.join(args.ffmpeg_dir, "ffmpeg"), video, outDir))
    if retn:
...
def create_video(dir):
    error = ""
    print('{} -r {} -i {}/%d.jpg -qscale:v 2 {}'.format(
        os.path.join(args.ffmpeg_dir, "ffmpeg"), args.fps, dir, args.output))
    retn = os.system('{} -r {} -i {}/%d.jpg -crf 17 -vcodec libx264 {}'.format(
        os.path.join(args.ffmpeg_dir, "ffmpeg"), args.fps, dir, args.output))
    if retn:
...
def main():
    ...
    if IS_WINDOWS:
        ...
        ctypes.windll.kernel32.SetFileAttributesW(
            extractionDir, FILE_ATTRIBUTE_HIDDEN)
    ...
    flowBackWarp = model.backWarp(
        videoFrames.dim[0], videoFrames.dim[1], device)
